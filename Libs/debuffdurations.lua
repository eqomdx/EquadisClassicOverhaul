--[[ **How long a debuff lasts, which 1.12 will not tell you.**

     `UnitDebuff` answers a texture and a stack count and stops. There is no
     duration and no expiry, so the only way to put a timer on a target's
     debuff is to know beforehand how long that spell lasts and to have seen it
     land.

     This is the knowing-beforehand half: a table of spell name to rank to
     seconds. It comes from ShaguPlates, MIT licensed, with the notice kept
     beside it in `LICENSE-ShaguPlates.txt`. The auras module used to say a
     duration could only come from such a table and that not answering was more
     honest than answering from one -- which was true while there was no table.
     There is one now.

     Rank zero is the entry for a spell whose rank does not change its duration,
     which is most of them and every mob ability. ]]--
